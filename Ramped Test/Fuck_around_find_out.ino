...
// Main Routine
void loop(){ 
  ramp_motors(0,delta-offset,0,delta);
  delay(5000);
  ramp_motors(delta-offset,0,delta,0);
  delay(5000);
...
void ramp_motors(int start_value_left, int end_value_left, int start_value_right, int end_value_right){
  if((start_value_left-end_value_left > 0) && (start_value_right-end_value_right > 0)){ // deceleration
    amplitude_left = (start_value_left - end_value_left)/2.0;
    amplitude_right = (start_value_right - end_value_right)/2.0;
    for(float t=(3.14159*2); t<((3.14159)*3);t+=0.01){ // timestep set
      runMotors(amplitude_left*sin(t/2.0)+(end_value_left+amplitude_left),amplitude_right*sin(t/2.0)+(end_value_right+amplitude_right));
      delay(5);
    }
  }
  if((start_value_left-end_value_left < 0) && (start_value_right-end_value_right <0)){
    amplitude_left = (end_value_left - start_value_left)/2.0;
    amplitude_right = (end_value_right - start_value_right)/2.0;
    for(float t=-3.14159; t<3.14159;t+=0.01){
      runMotors(amplitude_left*sin(t/2.0)+(start_value_left+amplitude_left),amplitude_right*sin(t/2.0)+(start_value_right+amplitude_right));
      delay(5);
    }
  }
}
